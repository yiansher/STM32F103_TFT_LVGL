...
void lv_port_disp_init(void)
{
    ...
    static lv_color_t buf_1_1[MY_DISP_HOR_RES * ROW_BUFFER]; /*A buffer for 10 rows*/
    lv_disp_set_draw_buffers(disp, buf_1_1, NULL, sizeof(buf_1_1), LV_DISP_RENDER_MODE_PARTIAL);

    //    /* Example 2
    //     * Two buffers for partial rendering
    //     * In flush_cb DMA or similar hardware should be used to update the display in the background.*/
    //    static lv_color_t buf_2_1[MY_DISP_HOR_RES * ROW_BUFFER];
    //    static lv_color_t buf_2_2[MY_DISP_HOR_RES * ROW_BUFFER];
...
void my_flush_cb(lv_disp_t *disp_drv, const lv_area_t *area, lv_color_t *px_map)
{
    uint8_t idx = 0;
    bool flag = true;
    ...
    uint16_t curNum = 0;
    uint16_t *colors;
    colors = (uint16_t *)px_map;
    uint32_t num = (area->x2 - area->x1 + 1) * (area->y2 - area->y1 + 1) * LV_COLOR_FORMAT_NATIVE_ALPHA_OFS;
    Add_set(area->x1, area->y1, area->x2, area->y2);
    SPI_CS_0;
    while (flag)
    {
        if (num > maxNum)
...
static void disp_flush(lv_disp_t *disp_drv, const lv_area_t *area, lv_color_t *px_map)
{
    if (disp_flush_enabled)
    {
        ...
        Add_set(area->x1, area->y1, area->x2, area->y2);
        uint32_t num = (area->x2 - area->x1) * (area->y2 - area->y1) * (LV_COLOR_FORMAT_NATIVE_ALPHA_OFS), num1 = 0;
        uint8_t t = 1;
        uint8_t px_arr[2] = {(px_map->red << 3 | px_map->green >> 3), (px_map->green << 5 | px_map->blue)};
        SPI_CS_0;
        while (t)
        {
            if (num > 2)
            ...
            while (!__HAL_SPI_GET_FLAG(&hspi1, SPI_FLAG_TXE))
                ;
            HAL_SPI_Transmit_DMA(&hspi1, (uint8_t *)px_arr, num1);
